...
export function readMap(day: number, test = false, delimeter: string = ''): string[][] {
  const lines = readInput(day, test).split('\n');
  return lines.map(l => l.split(delimeter).map(i => i));
}
export function readNumberMap(day: number, test = false, delimeter: string = ''): number[][] {
  const lines = readInput(day, test).split('\n');
  return lines.map(l => l.split(delimeter).map(i => Number(i)));
}
...
export const parseNumbers = (line: string, delimeter = ' '): number[] => line.split(delimeter).map(Number);
