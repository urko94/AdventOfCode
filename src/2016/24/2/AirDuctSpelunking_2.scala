...
object AirDuctSpelunking_2 {
  ...
  def main(args: Array[String]) {
    ...
    val shortestCircle = makeShortestCircle(locations, paths, 0)
    println("Shortest route - return to 0")
    println(shortestCircle)

  }

  def makeShortestCircle(locations: Map[Int, Location], paths: Seq[Path], start: Int): Int = {
    if (locations.contains(start)) {
      val routes = allCircles(locations.removed(start), paths, start, 0, 0)
      routes.min
    }
    else {
  ...
  def allCircles(locations: Map[Int, Location], paths: Seq[Path], previousLocation: Int, lastLocation: Int, steps: Int): Seq[Int] = {

    locations.keys.foldLeft(Seq[Int]())((acc, locationId) => {
      val pathLength = lengthBetweenLocations(previousLocation, locationId, paths)
      if (locations.size == 1) {
        val lastStepLength = lengthBetweenLocations(locationId, lastLocation, paths)
        acc :+ (steps + pathLength + lastStepLength)
      }
      else if ((locations.size > 1) && (acc.size == 0 || acc.min > (steps + pathLength))) {
        acc ++ allCircles(locations.removed(locationId), paths, locationId, lastLocation, (steps + pathLength))
      }
      else {
        acc
      }
